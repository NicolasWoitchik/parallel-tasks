...
const glob = globModule as jest.Mocked<typeof globModule>
...
describe('FileDiscoveryService - Essential Tests', () => {
  ...
  describe('🔥 Edge Cases', () => {
    it('should handle null patterns gracefully', () => {
      expect(() => service.discoverFiles(null as unknown as string[])).toThrow()
    })

    it('should handle undefined patterns gracefully', () => {
      expect(() => service.discoverFiles(undefined as unknown as string[])).toThrow()
    })
    ...
    it('should handle patterns that return undefined paths', () => {
      mockGlobSync.mockReturnValue([undefined, '/valid/path.ts', null] as unknown as string[])
